...
class ContactsUtility private constructor(context: Context){
    ...
    interface OnContactsListener {
        fun onReceivedContacts(contactsLst: List<ContactsBean>)
    }
    ...
    fun getContacts(listener: OnContactsListener) {
        val contactsLst = ArrayList<ContactsBean>()
        DOApplication._INSTANCE.getExecutorService().execute(object : Runnable {
            override fun run() {
                ...
                DOApplication._INSTANCE.getMainHandler().post(object : Runnable {
                    override fun run() {
                        listener.onReceivedContacts(contactsLst)
                    }
                })
            }
        })
